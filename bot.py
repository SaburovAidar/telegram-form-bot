...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.job_queue:
        jobs = context.job_queue.get_jobs_by_name("reminder_" + str(user.id))
        if not jobs:
            context.job_queue.run_once(
                reminder_job,
                when=timedelta(days=1),
                data={"chat_id": user.id, "name": user.first_name},
                name="reminder_" + str(user.id),
            )
...
В открытый доступ не выкладываю — работаю конфиденциально с каждым клиентом 🤝

Напишите мне в личные сообщения — покажу отзывы любого характера!\n\n"
                "📱 @OlegSergeevichGibdd\n"
                "━━━━━━━━━━━━━━━━"
            ),
            parse_mode="Markdown",
...
async def admin_panel_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "adm_close":
        ...
        if not source_stats:
            ...
            text += "Всего ссылок: *" + str(len(source_stats)) + "*\n\nВыберите ссылку:"
        ...
        bot_info = await context.bot.get_me()
        bot_username = bot_info.username
        link = "https://t.me/" + bot_username + "?start=SRC_" + src
...
async def admin_enter_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "create_link":
        ...
        bot_info = await context.bot.get_me()
        bot_username = bot_info.username
        link = "https://t.me/" + bot_username + "?start=SRC_" + source_name
        count = len(source_stats[source_name])
        kb = [[InlineKeyboardButton("◀️ К ссылкам", callback_data="adm_links")]]
        await update.message.reply_text(
            "✅ *Ссылка создана!*\n"
            "━━━━━━━━━━━━━━━━\n"
        ...
        await update.message.reply_text(
            "✅ Клиент: *" + context.user_data["target"] + "*\n\nВыберите услугу:",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup(kb),
        )
